...
class Facade_game:
    def __init__(self, game_color, x, y, hero:Actor):
        self.game = None
    ...
    def f_lauch(self):
        if self.game_color == 'white':
            self.game = WhiteGame()
        elif self.game_color == 'black':
            self.game = BlackGame()
        elif self.game_color == 'yellow':
            self.game = YellowGame()
        else:
            raise ValueError('Invalid color')
    
        self.game.ready()
        self.game.setDisplay(self.x, self.y)
        self.game.drawEdges()
        self.game.setHero(self.hero)
        self.game.launch()
         
hero = Hero(0, 0)
hero.setName("Berserker")
hero.setSkill("Volcano Burst")

game = Facade_game('black', 800, 600, hero)
game.f_lauch()
